import 'package:flutter/material.dart';
import 'package:foodordering/core/data/repositories/country_repositories.dart';
import 'package:foodordering/ui/views/map_view/map_controller.dart';
import 'package:get/get.dart';
import 'package:google_maps_flutter/google_maps_flutter.dart';
import 'package:location/location.dart';

class MapView extends StatefulWidget {
  const MapView({super.key, required this.currentLocation});
  final LocationData currentLocation;

  @override
  State<MapView> createState() => MapViewState();
}

class MapViewState extends State<MapView> {
  @override
  Widget build(BuildContext context) {
    return GetBuilder(
        init: MapController(widget.currentLocation),
        builder: (mapController) {
          return Scaffold(
            body: GoogleMap(
              mapType: MapType.normal,
              initialCameraPosition: mapController.initalCameraPosition,
              onMapCreated: (GoogleMapController controller) {
                mapController.controller.complete(controller);

                mapController.addMarker(
                    imageName: 'pin.jpg',
                    markerId: 'Current',
                    position: LatLng(
                        mapController.currentLocation.latitude ??
                            37.43296265331129,
                        mapController.currentLocation.longitude ??
                            -122.08832357078792));

                mapController.selectedLocation = LatLng(
                    mapController.currentLocation.latitude ?? 37.43296265331129,
                    mapController.currentLocation.longitude ??
                        -122.08832357078792);
                mapController.markerAllCountry();

                mapController.getStreetName();
              },
              markers: mapController.markers,
              onTap: (latlong) async {
                mapController.selectedLocation = latlong;

                mapController.addMarker(
                    imageName: 'pin.jpg',
                    markerId: 'Current',
                    position: LatLng(latlong.latitude, latlong.longitude));

                mapController.getStreetName();
              },
            ),
            floatingActionButton: FloatingActionButton.extended(
              onPressed: () {},
              label: Text(mapController.streetName.value),
              icon: const Icon(Icons.directions_boat),
            ),
          );
        });
  }
}
//! THIS MADE BY ME --
// class MapViewState extends State<MapView> {
//   late MapController mapController;

//   @override
//   void initState() {
//     mapController = Get.put(MapController(widget.currentLocation));
//     super.initState();
//   }

//   @override
//   Widget build(BuildContext context) {
//     return Scaffold(
//       body: Obx(
//         () => GoogleMap(
//           mapType: MapType.normal,
//           initialCameraPosition: mapController.initalCameraPosition,
//           onMapCreated: (GoogleMapController controller) {
//             mapController.controllerCompleter.complete(controller);
//             mapController.addToMarker();
//           },
//           markers: Set<Marker>.of(mapController.markers),
//           onTap: (latlong) {
//             mapController.selectedLocation.value = latlong;
//             mapController.addToMarker();
//           },
//         ),
//       ),
//       floatingActionButton: Obx(() => FloatingActionButton.extended(
//             onPressed: mapController.goToTheLake,
//             label: Text(mapController.selectedLocationStreet.value),
//             icon: const Icon(Icons.directions_boat),
//           )),
//     );
//   }
// }




